...
public:
	...
	bool operator==(String& ref){
		cout << getStringLen(ref) << " " << getStringLen(*this) << endl;
		cout << ref.string << "! " << string << "! " << endl;
		
		if(getStringLen(ref) != getStringLen(*this)){
			return 0;
		}
		bool check = 1;
		bool formalCheck = 1;
		
		for(int i = 0; i < getStringLen(*this); i++){
			formalCheck = check;
			if(formalCheck == 0){
				return 0;
			}
			if(string[i] == ref.string[i]){
				check = formalCheck && check;;
			}
			else{
				check = 0;
			}
